...
exports.resetPassword=asyncErrorHandler(async(req,res,next)=>{
    ...
    //We are logging i user with an token
    res.status(200).json({
        status:"success",
        token:loginToken
    })
...
exports.updatePassword=asyncErrorHandler(async(req,res,next)=>{
    ...
    /*Until user.save() is executed, the changes made to the user object 
    (such as updating the password and confirmPassword fields) exist only in memory and are 
    not yet persisted to the database. Once user.save() is executed, 
    the changes are written to the database, making them persistent.
    */
   await user.save()
