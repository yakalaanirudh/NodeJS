...
res.cookie('jwt',token,{
    ...
    secure:true,                    //cookie is set only on https connection
    httpOnly:true                   //Browser cant modify http only preventing cross site scripting attack
})
*/
const createSendResponse=(user,statusCode,res)=>{
    const token=signToken(user._id)
    ...
    //This adds the property when environemnt is production
    if(process.env.NODE_ENV==='production'){options.secure=true}    

    res.cookie('jwt',token,options)

    //no password in response
    //Since we are not saving it password will not be affected in DB Storage the field
    //itself will be  just be not shown in response
...
exports.resetPassword=asyncErrorHandler(async(req,res,next)=>{
    //CHECK IF USER EXISTS WITH GIVEN TOKEN AND ITF THAT TOKEN HASNT EXPIRED
    //we take the token from req body encrypt it and compare it to encrypted token in db
