...
exports.getMovieStats = async (req, res) => {
    try{
        //We pass [] to the aggreagte method to signify different stages
        const stats = await Movie.aggregate([
            { $match: {ratings: {$gte: 4.5}}},      //1st Stage match where ratings greater than 4.5 
            //Now all movies with rating greater than 4.5 is stored in match variable    
            { $group: {                             //2nd stage it is applied on result of 1st stage
                _id: '$releaseYear',
                ...
                movieCount: { $sum: 1}          //Initially zero when a document passes 1 then 2(1+1) then 3(2+1)
            }},
            //3rd stage is applied on groups not documents
            { $sort: { minPrice: 1}}                //3rd stage applied on result of 2nd stage ascending order 
            //4th stage also applied on groups not documents
            //{ $match: {maxPrice: {$gte: 60}}}     //4th stage applied on result of 3rd stage maxprice greater than 60
        ]);

        res.status(200).json({
            status: 'success',
            count: stats.length,        //The result from first stage number of movies
            data: {
                stats
            }
        });
...
exports.getMovieByGenre = async (req, res) => {
    try{
        const genre = req.params.genre;         //get genre from req
        const movies = await Movie.aggregate([
            //unwind creates a document for each element in the array specified
            {$unwind: '$genres'},//1st stageA movie has multiple genres in the genre array this creates a document for each genre
            {$group: {   //2nd stage group by genre, count number for each genre and push name of movie to a array named movies
                _id: '$genres',
                movieCount: { $sum: 1},
                movies: {$push: '$name'}, 
            }},
            //3rd stage  add a field named genre in 2nd stage each group is ided by genre so use the id as field value
            {$addFields: {genre: "$_id"}},
            {$project: {_id: 0}},       //4th stage dont show id in the output object
            {$sort: {movieCount: -1}},  //5th stage sort desc
            //{$limit: 6}               //6th stage limit to 6
            //{$match: {genre: genre}}  //7th stage match show genre where it matches req.genre
        ]);
